...
function CHoldoutGameMode:InitGameMode()
	...
	GameRules:SetHeroRespawnEnabled( false ) -- allows respawn
	GameRules:SetUseUniversalShopMode( true ) -- shop all shops for everyone
...
function CHoldoutGameMode:_CheckForDefeat()
	...
	-- For player alive state (no resawn but team-mates can revive)
	for nPlayerID = 0, DOTA_MAX_TEAM_PLAYERS-1 do
		if PlayerResource:GetTeam( nPlayerID ) == DOTA_TEAM_GOODGUYS then
