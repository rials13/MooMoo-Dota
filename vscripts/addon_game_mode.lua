...
function Activate()
	...
	-- Listen to a player levelup so we can adjust Ability Points for 50 levels
	ListenToGameEvent( "dota_player_gained_level", Dynamic_Wrap(CHoldoutGameMode, "PlayerGainedLevel" ), self)
	-- Listen to all players inventory to see if items are made that we only want one of in the game
	ListenToGameEvent( "dota_item_picked_up", Dynamic_Wrap(CHoldoutGameMode, "InventoryItemAdded" ), self )
	ListenToGameEvent( "dota_item_purchased", Dynamic_Wrap(CHoldoutGameMode, "InventoryItemAdded" ), self )
	GameRules.moo_tshirt_made = false
...
function CHoldoutGameMode:InitGameMode()
	...
	GameRules:SetHeroRespawnEnabled( true ) -- allows respawn
	GameRules:SetUseUniversalShopMode( false ) -- shop all shops everywhere
...
--	GameRules:GetGameModeEntity():SetHUDVisible( DOTA_HUD_VISIBILITY_SHOP_SUGGESTEDITEMS, false )
	...
	--added by me for quick testing
	--GameSetup:init()

	-- get Moo Moo to level up reactive armor
	--[[local abilityInd = self._entAncient:GetAbilityByIndex( 0 )
	abilityInd:SetLevel( 1 )
	abilityInd = self._entAncient:GetAbilityByIndex( 1 )
	abilityInd:SetLevel( 1 )
	abilityInd = self._entAncient:GetAbilityByIndex( 2 )
	abilityInd:SetLevel( 1 )

	print("set level of Moo Moo's ability")
	]]
...
-- set a variable used to track spawn locations _vRandomSpawnsList is not 0 index'ed (as setup runs 1-4)
local spawnIndex = 1
function CHoldoutGameMode:ChooseOrderedSpawnInfo()
	if #self._vRandomSpawnsList == 0 then
		error( "Attempt to choose a random spawn, but no random spawns are specified in the data." )
		return nil
	end
	-- we want to spawn at all 4 locations: return the whole span zone array
	if spawnIndex > #self._vRandomSpawnsList then
		spawnIndex = 1 --loop index back to start at end of array
		print("reset spawn index")
	end

	local spawn = self._vRandomSpawnsList[ spawnIndex ]
...
function CHoldoutGameMode:_CheckForDefeat()
	...
	local mooMoo = self._entAncient
	if mooMoo and mooMoo:IsAlive() then
...
function CHoldoutGameMode:PlayerGainedLevel()
...
-- Checks player's inventory to see if certain items are made
function CHoldoutGameMode:InventoryItemAdded()
	local item =  self.itemname
	local hero = PlayerResource:GetSelectedHeroEntity( self.PlayerID )
		print(item)
	-- item_moo_tshirt = item_orb_of_fire + item_orb_of_frost + item_orb_of_darkness + item_orb_of_lightning + item_mithril_shirt
	if hero:HasItemInInventory("item_orb_of_fire") and hero:HasItemInInventory("item_orb_of_frost") and hero:HasItemInInventory("item_orb_of_darkness") and hero:HasItemInInventory("item_orb_of_lightning") and hero:HasItemInInventory("item_mithril_shirt") then
		print("item_moo_tshirt")
		if GameRules.moo_tshirt_made == false then
			hero:AddItemByName("item_recipe_moo_tshirt")
			GameRules.moo_tshirt_made = true
		end
	end
	-- item_halberd_axe = item_orb_of_fire + item_orb_of_frost + item_orb_of_darkness + item_orb_of_lightning + item_imperial_axe
end
